...
# genome_path = '/ssd/zhaohan/v4/v4.fa'
...
for file in all_files:
    ...
    if file_path.endswith('_sort.bam') or file_path.endswith('.sort.bam'):
...
for file in list_sra_files_1:
    ...
    if '_' in file_sra_name:
        pre_file_name = file_sra_name.split('_')[0]
        cmd_1 = 'sentieon driver -t 70 -i {}_sort.bam'.format(
            pre_file_name) + ' --algo LocusCollector --fun score_info {}.score'.format(pre_file_name)
        cmd_2 = 'sentieon driver -t 70 -i {}_sort.bam'.format(
            pre_file_name) + ' --algo Dedup --rmdup --score_info {}.score'.format(
            pre_file_name) + ' --metrics {}.dedup.metrics'.format(pre_file_name) + ' {}.deduped.bam'.format(
            pre_file_name)
        ...
        pre_file_name = file_sra_name.split('.')[0]
        cmd_1 = 'sentieon driver -t 70 -i {}.sort.bam'.format(
            pre_file_name) + ' --algo LocusCollector --fun score_info {}.score'.format(pre_file_name)
        cmd_2 = 'sentieon driver -t 70 -i {}.sort.bam'.format(
            pre_file_name) + ' --algo Dedup --rmdup --score_info {}.score'.format(
            pre_file_name) + ' --metrics {}.dedup.metrics'.format(pre_file_name) + ' {}.deduped.bam'.format(
            pre_file_name)
